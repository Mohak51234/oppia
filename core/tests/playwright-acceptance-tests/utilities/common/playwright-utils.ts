...
export class BaseUser {
  ...
  async expectScreenshotToMatch(
    ...
    console.log('CI env check:', process.env.CI);
    ...
    if (
      ...
      if (runningInCI) {
        errorMessage +=
          ...
          ' Generate it via the update-snapshots workflow (covers dev + prod), or run locally with' +
          ' --update_snapshots and commit the resulting screenshot.';
      } else {
        errorMessage +=
